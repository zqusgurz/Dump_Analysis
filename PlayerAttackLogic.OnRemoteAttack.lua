return function (self,user,attackType,attacks,mobCount,damagePerMob,skillId,skillLevel,action,masteryLevel,left,actionDelay,remoteAttackInfo,fromUserId) 
...
local damageInfos = self:MakeDamageInfos(user, attacks, mobCount, skillId, skillLevel, left, false)
local hitEffects = self:MakeHitEffects(user, attackType, attacks, mobCount, skillId, skillLevel, action)
...
self:ProcessHitDamageInfo(skillId, mobCount, damagePerMob, attacks, damageInfos, hitEffects, false)
if (attackType ~= _AttackType.Magic) then
	ac:RegisterAfterimage(action, masteryLevel, left, actionDelay, sfx)
end

self:PlayAction(user, body, nil, actionName, ac:GetWeaponAttackSpeed(magic))
end